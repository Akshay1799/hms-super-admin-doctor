...
export async function allocateBed(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (bed.status !== 'Available' && bed.status !== 'Reserved') {
      throw new AppError(`Bed is currently ${bed.status}`, 400);
    }
    ...
    sendSuccess(res, bedAllocation, 'Bed allocated successfully', 201);
  } catch (err) {
    next(err);
  }
}
...
export async function releaseBed(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const allocation = await BedAllocation.findOne({ _id: allocationId, tenantId, hospitalId });
    if (!allocation) throw new AppError('Bed Allocation not found', 404);
    if (allocation.status === 'Released') throw new AppError('Bed already released', 400);

    const bed = await Bed.findOne({ _id: allocation.bedId, tenantId, hospitalId });
    if (!bed) throw new AppError('Bed not found', 404);
