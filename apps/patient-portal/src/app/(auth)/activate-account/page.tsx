...
function ActivateAccountForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err.message || "Unable to activate this account. Please contact hospital administration.");
    } finally {
  ...
  if (!token) {
    return (
      <section className="w-full max-w-sm space-y-4 text-center">
        ...
        <p className="text-sm text-muted-foreground">This link is missing its activation token. Please contact your hospital administrator.</p>
  ...
  return (
    <section className="w-full max-w-sm space-y-6">
      <div className="space-y-2">
        <h1 className="text-2xl font-bold text-foreground">Activate your Patient Portal</h1>
        <p className="text-sm text-muted-foreground">Set a password to access your health portal account.</p>
      </div>
