...
export function AuditLogsTab({ details }: AuditLogsTabProps) {
  ...
  const timelineEvents = auditLogs.map((log) => ({
    ...
    time: log.timestamp,
    type: (log.action.toLowerCase().includes("suspend") || log.action.toLowerCase().includes("delete")
      ? "destructive"
      : log.action.toLowerCase().includes("register") || log.action.toLowerCase().includes("create")
      ? "success"
      : "info") as any,
  }));
  ...
  return (
    <div className="bg-card border border-border rounded-[var(--radius-card)] p-6 shadow-sm space-y-6 animate-in fade-in duration-200">
      <div>
        <h2 className="text-lg font-bold text-foreground">Hospital Audit Trail</h2>
        <p className="text-sm text-muted-foreground mt-0.5">
          A chronological log of configuration changes, capacity revisions, and state modifications.
        </p>
      </div>
      ...
      {timelineEvents.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground">
          ...
          <p className="text-xs mt-0.5">No activities have been recorded for this hospital node yet.</p>
        </div>
      ) : (
        <div className="pl-2">
