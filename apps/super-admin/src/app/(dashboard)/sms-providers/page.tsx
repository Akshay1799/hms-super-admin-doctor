...
import { useSmsProviders, useUpdateSmsProviders } from "@/features/integrations/hooks/use-integrations";
...
export default function SmsProvidersPage() {
  ...
  const updateProviders = useUpdateSmsProviders();
  ...
  const handleAddSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateProviders.mutate([...providers, newProvider], {
      onSuccess: () => {
        toast.success("New SMS provider added successfully.");
  ...
  return (
    <PageContainer>
      <PageHeader 
        title="SMS Providers" 
        description="Configure SMS gateway integrations for patient and staff notifications" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-2xl p-5 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center pb-2 border-b border-border">
              <h3 className="text-sm font-bold text-foreground">Add SMS Provider</h3>
