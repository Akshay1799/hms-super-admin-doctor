...
import { useEmailProviders, useUpdateEmailProviders } from "@/features/integrations/hooks/use-integrations";
...
export default function EmailProvidersPage() {
  ...
  const updateProviders = useUpdateEmailProviders();
  ...
  const handleAddSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateProviders.mutate([...providers, newProvider], {
      onSuccess: () => {
        toast.success("New email provider added successfully.");
  ...
  return (
    <PageContainer>
      <PageHeader 
        ...
        description="Configure SMTP and transactional email provider settings" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-2xl p-5 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center pb-2 border-b border-border">
              <h3 className="text-sm font-bold text-foreground">Add Email Provider</h3>
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input required type="email" name="fromAddress" placeholder="no-reply@hospital.com" className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-2xl p-5 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleConfigureSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-muted-foreground uppercase">From Email Address</label>
                <input required type="email" name="fromAddress" defaultValue={selectedProvider.fromAddress} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              <div className="pt-2 border-t border-border flex justify-end gap-2">
